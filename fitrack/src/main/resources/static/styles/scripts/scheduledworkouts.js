...
function estimateBurnedCalories() {
    ...
    if (!workoutName || !duration) {
        alert('Please enter workout name and duration to estimate calories.');
        return;
    }
    ...
    fetch('/workouts/estimate-calories', {
        ...
        body: JSON.stringify({ workoutName: workoutName, duration: parseFloat(duration) })
    })
    .then(response => {
        if (!response.ok) {
            return response.text().then(text => { 
                throw new Error(text || 'Failed to estimate burned calories. Status: ' + response.status);
            });
        }
    ...
    .then(data => {
        if (data && data.calories !== undefined) {
            document.getElementById('caloriesBurned').value = Math.round(data.calories);
        } else {
            throw new Error('Invalid response format from calorie estimation.');
        }
    })
    .catch(error => {
        console.error('Error estimating burned calories:', error);
        alert('Error estimating burned calories: ' + error.message);
    })
    .finally(() => {
